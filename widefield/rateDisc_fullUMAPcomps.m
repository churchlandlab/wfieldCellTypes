...
mouseIdx = [];
groupIdx = [];
pcCount = 20; %nr of components from each recording
...
allC = cell(1,length(animals)); %temporal components
...
for iAnimals = 1 : length(animals)
    ...
    for iRecs = 1 : size(recs,1)
        ...
        load([fPath 'allAC_200.mat'], 'A', 'C');
        C = C(1:min([pcCount,size(C,1)]), :);
        A = A(:,:, 1:min([pcCount size(A,3)]));
        ...
        for x = 1 : size(A,3)
            cImg = arrayFilter(A(:,:,x), 2, 2, 2);
            A(:,:,x) = imadjust(cImg,[min(cImg(:)) prctile(cImg(:), 99)]);
        end
        ...
        % align temporal components to task episodes
            ...
            for x = 1 : length(temp)
                temp{x} = temp{x} - nanmean(temp{x}(:, 1:15),2); %baseline correction
            end
...
save([umapPath cTime '_fullA.mat'], 'X', 'Y', 'shrinkMask', 'recIdx', 'mouseIdx', 'groupIdx', 'trialCnt', 'animals','-v7.3');
